...
const ReserveBook = () => {
    ...
    useEffect(() => {
        ...
        const fetchBooks = async () => {
            try {
                const booksCollectionRef = collection(db, 'reserve');
    ...
    const handleReserve = async (bookId, idx, bookstatus) => {
        try {
            const bookRef = doc(db, 'reserve', bookId);

            const updatedBooks = books.map((book) =>
                book.id === bookId ? { ...book, status: bookstatus === "Reserve" ? "Reserved" : "Reserve" } : book
            );
            ...
            if (myBooks.length > 0) {
                ...
                await Promise.all([
                    ...
                    updateDoc(doc(db, "reserve", bookId), {
                        status: bookstatus === "Reserve" ? "Reserved" : "Reserve",
                    }),
                ]);
        ...
        <ScrollView style={styles.bookItem}>
            <View style={styles.bookCont}>
                ...
                <View style={styles.descCont}>
                    ...
                    <TouchableOpacity onPress={() => handleReserve(item.id, books.indexOf(item), item.status)}>
